...
static PyObject *AerospikePredicates_Equals(PyObject *self, PyObject *args)
{
    ...
    if (PyLong_Check(py_val) || PyLong_Check(py_val)) {
        return Py_BuildValue("iiOO", AS_PREDICATE_EQUAL, AS_INDEX_NUMERIC,
                             py_bin, py_val);
    }
    else if (PyUnicode_Check(py_val) || PyUnicode_Check(py_val)) {
                                                   ...
                                                   PyObject *args)
{
    ...
    if ((PyLong_Check(py_min) || PyLong_Check(py_min)) &&
        (PyLong_Check(py_max) || PyLong_Check(py_max))) {
                                                             ...
                                                             PyObject *args)
{
    ...
    if (PyUnicode_Check(py_shape) || PyUnicode_Check(py_shape)) {
                                                              ...
                                                              PyObject *args)
{
    ...
    if (PyUnicode_Check(py_point) || PyUnicode_Check(py_point)) {
        return Py_BuildValue("iiOOOO", AS_PREDICATE_RANGE, AS_INDEX_GEO2DSPHERE,
